...
module Ctx = struct
  ...
  let get_cstor_type name = 
    begin match StrTbl.find_opt t.constructors name with
    | Some (ty, _) -> ty
    | None -> raise (UnsupportedQuery ("trying to get the type of " ^ name ^", but it is not a constructor!"))
  end
  ...
  let get_selector_output_ty name = 
    begin match (StrTbl.find_opt t.selectors name) with
      | Some (ty, _, _ ) -> ty
      | None -> raise (UnsupportedQuery ("Error: " ^ name ^ " is not a selector."))
    end
