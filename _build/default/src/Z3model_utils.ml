    ...
    match stack with
      ...
      | (term, term_ty, selection_depth, selector_list, guards) :: rest
        ...
        let one_applications = List.filter_map 
                                  (fun selector -> if ((ty_equal (fst selector) term_ty) && (ty_equal original_term_ty (Ctx.get_selector_output_ty (snd (snd selector)))))
                                                   then Some (Z3.Expr.mk_app ctx (fst (snd selector)) [z3_term]) else None) z3_selector_list in 
